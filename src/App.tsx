...
import { ColumnProps, ParameterProps } from "./utils/interface.tsx";
...
const App = () => {
  ...
  const columnProps : ColumnProps[]= [
    ...
    { columnKey:"gen der", label: "Gender",type :'text' ,isEditable :true},
  ...
  useEffect(() => {
    setColumnState && setColumnState(columnProps);
    getData();  
  }, [parameter]);
  ...
  const handleEdit = (rowIndex, childIndex, columnKey, value) => {
    const updatedData : any = [...editedData];
  ...
  const handleSave = (rowIndex, childIndex, data, columnKeys) => {
    ...
    const editedRowData:any = editedData[rowIndex]?.[childIndex];
    if (editedRowData) {
    ...
    setEditedData((prevEditedData) => {
      const updatedEditedData: any = [...prevEditedData];
